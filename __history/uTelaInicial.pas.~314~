...
begin
  ...
  if (PesoAtual <> UltimoPeso) and (PesoAtual > 0) then
    begin
      ...
      ProdutoBancoDeDados := GetProduto;
      PrecoCodigoDeBarras := FormatFloat('0.00', ProdutoBancoDeDados.PrecoProduto*PesoAtual).Replace(',', '');
      ...
      try
        ...
        Self.FormResize(Self);
        Sleep(500);
        EnviarImpressao(GetNomeEstabelecimento, ProdutoBancoDeDados.NomeDoProduto, FormatFloat('0.000', PesoAtual), FloatToStr(ProdutoBancoDeDados.PrecoProduto), FormatFloat('0.00', ProdutoBancoDeDados.PrecoProduto*PesoAtual), GetCodigoDeBarras(PrecoCodigoDeBarras));
        Sleep(500);
...
function TfrmTelaPrincipal.GetProduto: TProduto;
var
  DadosArqIni: TDadosArqIni;
...
begin
  ...
  if not Query.IsEmpty then
    begin
      DadosProduto.NomeDoProduto := Query.FieldByName('NOMPRO').AsString;
